...
public class DiaryController {
    ...
    public ResponseEntity<ResponseDiary> updateDiary(@RequestHeader("Authorization") String token, @RequestBody RequestUpdate request, @RequestParam LocalDate date){
        try {
            String userToken = jwtUtility.bearerToken(token);
            Diary diary = diaryService.updateDiary(request.getContent(), userToken, date);
    ...
    public ResponseEntity<Void> deleteDiary(@RequestHeader("Authorization") String token, @RequestParam LocalDate date){
        try {
            String userToken = jwtUtility.bearerToken(token);
            diaryService.deleteDiary(date, userToken);
